...
- [ ] Deploy-on-merge-to-main added to the GitHub Actions workflow
      **Written and partly verified; deliberately left unchecked until it has actually run once.** `.github/workflows/ci.yml` gains a `deploy` job plus a second trigger (`push` to `Main`). It `needs: [lint, test, build, test-integration]`, so the same suite that gates a PR also gates production, and `if:` restricts it to pushes on `Main` - forks cannot push, so fork PRs can never reach it even though it uses secrets.
      ...
      - **Requires 3 one-time repo secrets from the user** (`DEPLOY_HOST`, `DEPLOY_HOST_KEY`, `DEPLOY_SSH_KEY`) - not something a commit can configure.
      - **Not yet verified end to end.** The key-auth-with-pinned-host-key step was rehearsed successfully; the `git reset --hard` + rebuild portion could not be rehearsed from this session (blocked by the sandbox's permission classifier, not worked around), so the first real merge to `Main` is its first true run.
